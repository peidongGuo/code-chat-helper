...
def review_pr():
    logging.info("Received user request")
    if not validate_signature(request):
        abort(401, 'Invalid signature')
    logging.info("Webhook signature validated")

    event = request.get_json()
    logging.info(f"Webhook event type: {event['action']}")
    ...
    logging.info(f"Summarizing {len(reviews)} reviews into final review")
    ...
    **[Note]** 
    ...
    logging.info("Final review prepared")

    logging.info("Translating review to Chinese")
    translate_messages = [
        {"role": "user",
